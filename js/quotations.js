function loadQuotations() {
  ...
  if (quotations.length === 0) {
    listContainer.innerHTML = "<p class='text-gray-400'>No quotations found.</p>";
    return;
  }

  listContainer.innerHTML = "";

  quotations.reverse().forEach((q, index) => {

    const statusColor =
      q.status === "Draft" ? "bg-gray-500" :
      q.status === "Sent" ? "bg-yellow-500" :
      q.status.includes("Confirmed") ? "bg-green-600" :
      "bg-red-500";
